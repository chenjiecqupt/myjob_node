/*!
 *
 * Created by chenjie && liaominhe on 2017/8/9.
 */
$(function() {
  var self;
  ...
  $.extend(CanvasTool.prototype, {
    ...
    switch:function(){
      ...
      var index = 0;
      $('.image-labeler span').eq(0).html(index+1);
      $('.image-labeler span').eq(1).html(this.setting.srcArr.length);
      self.status.index = index;
      $('.switch_prev').off('click').on('click',function(){
        if($('.large-and-small').hasClass('small')){
          $('.large-and-small').click();
        }
        console.log(self.status.cancelList.length);
        console.log('cancelIndex: '+self.status.cancelIndex);
        if(self.status.cancelIndex>0){
    ...
    srcSubmit:function(){
      if(self.status.cancelIndex>0){
        self.setting.srcArr[self.status.index] = self.status.cancelList[self.status.cancelList.length-1];
        self.status.url[self.status.index] = self.setting.srcArr[self.status.index];
      }
